...
この手法では，GPT-3.5やGPT-4などのLLMに対して特定の受験者レベル（1〜5の5段階）の受験者として振る舞うよう指示するプロンプトを設計し，多肢選択問題への回答を生成させる．
LLMへの入力はシステムプロンプト（模擬する受験者レベルと出力形式の指定項目の問題文と選択肢のテキスト），読解問題の場合は読解パッセージから構成される．LLMに出力させる内容は項目の難易度レベル，選択した回答の説明（その受験者レベルの受験者が辿る思考過程や誤解を含む），選択した回答のインデックスで構成されるJSON形式である．
...
% そこで本研究では，仮想受験者の受験者レベルをCEFR Global Scaleに基づいて設定し，読解問題の正答に必要な知識・技能といった受験者プロフィールを精緻化してプロンプトに与えることで，
% 高性能なLLMモデルであっても，単調増加性や難易度依存性の観点からより人間に近い受験者反応を模倣できる手法を提案する．
% 具体的には，語彙レベル，文法・語法理解，読解対象文および設問に対する情報処理の各観点について，CEFR Global ScaleのA1からC2の6段階の受験者レベルに対応する受験者の具体的な振る舞いや，知識・技能に関する制限を受験者プロフィールとして構築する．
% この受験者プロフィールを読解対象文および設問とともにプロンプトに与えることで，LLMに指定した受験者レベルの受験者として振る舞わせる．
